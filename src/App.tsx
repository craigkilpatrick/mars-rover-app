...
function App() {
  ...
  const handleAddRover = async () => {
    try {
      ...
      setError('Failed to create rover: ' + (error instanceof Error ? error.message : String(error)));
    }
  };

  const handleDeleteRover = async (id: string) => {
    try {
      ...
      setError('Failed to delete rover: ' + (error instanceof Error ? error.message : String(error)));
    }
  };
  ...
  const handleSendCommands = async (commands: Command[]) => {
    ...
    try {
      ...
      setError('Failed to send commands: ' + (error instanceof Error ? error.message : String(error)));
    }
  };
